...
export class ContactClient {
     ...
     * @return Success
     */
    searchContacts(searchTerm: string | null): Observable<ContactModel[]> {
        let url_ = this.baseUrl + "/api/Contact/SearchContacts/{searchTerm}";
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processSearchContacts(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processSearchContacts(<any>response_);
                } catch (e) {
                    return <Observable<ContactModel[]>><any>_observableThrow(e);
                }
            } else
    ...
    protected processSearchContacts(response: HttpResponseBase): Observable<ContactModel[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
